        ...
        namespace drawable
        {
            ...
            SDL_Surface* BodyDrawer::performAdditionalRendering(SDL_Surface* picture, const utils::sdl::Area& sourceArea, const double& xScale, const double& yScale, const Uint8& alpha)
            {
                ...
                else
                {
                    ...
                    SDL_Rect srcArea({0, 0, frustumRenderingWidth, yOffset});
                    SDL_Rect dstArea({0, 0, frustumRenderingWidth, frustumRenderingHeight});
                    SDL_BlitSurface(frustumSurface, &srcArea, backgroundSurface, &dstArea);

                    srcArea = {0, 0, frustumRenderingWidth, yOffset};
                    dstArea = {0, frustumRenderingHeight - yOffset, frustumRenderingWidth, frustumRenderingHeight};
                    SDL_BlitSurface(frustumSurface, &srcArea, backgroundSurface, &dstArea);

                    srcArea = {0, 0, xOffset, frustumRenderingHeight - 2 * yOffset};
                    dstArea = {0, yOffset, frustumRenderingWidth, frustumRenderingHeight};
                    SDL_BlitSurface(frustumSurface, &srcArea, backgroundSurface, &dstArea);

                    srcArea = {0, 0, xOffset, frustumRenderingHeight - 2 * yOffset};
                    dstArea = {frustumRenderingWidth - xOffset, yOffset, frustumRenderingWidth, frustumRenderingHeight};
                    SDL_BlitSurface(frustumSurface, &srcArea, backgroundSurface, &dstArea);
